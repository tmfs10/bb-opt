...
if __name__ == "__main__":
	...
	train_loader2 = DataLoader(TensorDataset(data.train.inputs, data.train.labels),batch_size=batch_size,shuffle=False)
	test_loader = DataLoader(TensorDataset(data.test.inputs, data.test.labels),batch_size=batch_size,shuffle=False)
	top_loader = DataLoader(TensorDataset(data.top.inputs, data.top.labels),batch_size=batch_size,shuffle=False)
	val_loader = DataLoader(TensorDataset(data.val.inputs, data.val.labels),batch_size=batch_size,shuffle=False)
	...
	for epoch in range(epoch, epoch + args.epoch):
		...
		with torch.no_grad():
			...
			for batch in train_loader2:
				inputs, labels = batch
				means, variances = model(inputs)
				m=means if m is None else torch.cat([m,means],1)
				v=variances if v is None else torch.cat([v,variances],1)
