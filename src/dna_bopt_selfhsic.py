...
if len(sys.argv) < 11:
    print("Usage: python", sys.argv[0], "<init_train_epochs> <init_lr> <retrain_epochs> <retrain_lr> <ack_batch_size> <pred_weighting> <data dir> <filename file> <output dir> <suffix>")
    sys.exit(1)

init_train_epochs, train_lr, retrain_num_epochs, retrain_lr, ack_batch_size, pred_weighting, data_dir, filename_file, output_dir, suffix = sys.argv[1:]

if output_dir[-1] == "/":
    output_dir = output_dir[:-1]
output_dir = output_dir + "_selfhsic_" + suffix
        ...
        for ack_batch_size in [20]:
            ...
            with open(batch_output_dir + "/stats.txt", 'w') as f:
                for ack_iter in range(params.num_acks):
                    ...
                    mves_idx, best_hsic = bopt.acquire_batch_self_hsic(
                            params, 
                            preds, 
                            skip_idx_mves, 
                            mves_compute_batch_size, 
