...
for ack_iter in range(10):
    ...
    preds[:, list(skip_idx_pdts)]
    sorted_preds_idx = []
    for i in range(preds.shape[0]):
        sorted_preds_idx += [np.argsort(preds[i].numpy())]
    sorted_preds_idx = np.array(sorted_preds_idx)
    pdts_idx = np.unique(sorted_preds_idx[:, -top_k:])
    pdts_idx = np.choice(pdts_idx, params.ack_batch_size)
    ...
    ack_pdts = X[ei_idx]
    ack_pdts_vals = (Y[ei_idx]-float(train_label_mean))/float(train_label_std)
