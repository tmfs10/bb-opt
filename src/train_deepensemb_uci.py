...
from bb_opt.src.deep_ensemble_sid import (
    NNEnsemble,
    RandomNN,
)
...
def sample_uniform(out_size,min_val,max_val):
    low=np.tile(min_val-1,(out_size[0],1))
    hi=np.tile(max_val+1,(out_size[0],1))
    z=np.random.uniform(low=low,high=hi)
...
if __name__ == "__main__":
	...
	non_linearity = 'ReLU'
	#top_k_percent = 1
	data_root = "/cluster/geliu/bayesian/DropoutUncertaintyExps/UCI_Datasets"
	project = "data"
	...
	elif args.data_type =='uci':
		data = load_data_uci(data_root, project, dataset, n_train, n_val, args.percent, standardize_labels=True,random_state=args.seed, device=device)
	...
	train_newloss=[]
	epoch = 0
	min_val=data.train.inputs.min(0)[0].cpu().numpy()
	max_val=data.train.inputs.max(0)[0].cpu().numpy()
